...
Actions path is event-driven: the adopter calls the reusable workflow, `ax:go`
...
The Actions runtime authenticates as the `aixgo-code[bot]` GitHub App.
...
internal/config/      .github/aixgo.yml; refuses a missing gate; engine, review,
                      prDescription, attribution
internal/state/       ax: label lifecycle with mutual exclusion
...
internal/command/     comment commands (@<your-app> go | address | help)
internal/forge/dryrun/ records the GitHub writes a dry run skips
internal/attribution/ the Aixgo Code marker on generated commits and PRs
